...
class BiLSTM_AttentionModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size=3):
        ...
        # 自注意力机制
        self.attention1 = SelfAttention(hidden_size * 2)  # 用于LSTM1输出
        self.attention2 = SelfAttention(hidden_size * 2)  # 用于LSTM2输出
    ...
    def forward(self, x):
        # LSTM1 + Dropout1
        x, _ = self.lstm1(x)
        x = self.attention1(x)  # 自注意力机制
        x = self.dropout1(x)
        x = self.relu(x)

        # LSTM2 + Dropout2
        x, _ = self.lstm2(x)
        x = self.attention2(x)  # 自注意力机制
        x = self.dropout2(x)
        x = self.relu(x)
...
model = BiLSTM_AttentionModel(input_size=input_size, hidden_size=hidden_size).to(device)
...
train_model(model, train_loader, test_loader, epochs=1000)
